...
Next we run the sampler with 100000 iterations, using the BIC and liang_g1 priors.
...
mGamma_BIC %>% prepare_tbl %>% knitr::kable(caption="BIC")
mGamma_BIC %>% head(200) %>% ggplot_matrix
p_BIC <- apply(mGamma_BIC, 1, sum)
```
Comparing this against the exact results, we see that they are quite close.
```{r Exact results}
results_tbl <- parameters_tbl %>%
                  mutate(sampler_vip_mean=list(apply(sampler_result[[1]]$mGamma, 2, mean)),
                         exact_vip_mean=list(blma_result[[1]]$vinclusion_prob),
                         error=sum(sampler_vip_mean[[1]] - exact_vip_mean[[1]]))
results_tbl %>% View
```
