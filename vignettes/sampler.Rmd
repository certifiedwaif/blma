...
data_set <- function(name)
{
  ...
  if (name == "comCrime") {
    # Fill in
    comData <- bind_cols(Y %>% as.tibble, X %>% as.tibble)
    data(comData)
    Y <- comData[, 1:18]
    X <- comData[, 19:142]
    # Data preparation
  ...
  if (name == "QTL") {
	  response <- data(phe_simulat)
	  covariates <- data(gen_simulat)
...
parameters_tbl <- parameters_tbl %>%
                      mutate(sampler_mGamma=future_map(sampler_result, "mGamma", .progress = TRUE),
                             ...
                             error=future_map2_dbl(sampler_vip, blma_vip, ~sum((.x - .y)^2)), .progress = TRUE)
