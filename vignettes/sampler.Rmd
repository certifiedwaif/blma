...
data_set <- function(name)
{
  ...
  if (name == "buzz") {
    prefixes <- c("NCD", "BL", "NAD", "AI", "NAC", "ND", "CS", "AT", "NA",
                  "ADL", "AS(NA)", "AS(NAC)")
    TH_col_names <- cross2(0:7, prefixes) %>% map(rev) %>% map_chr(lift(paste, sep = "_"))
...
parameters_tbl <- parameters_tbl %>%
                      mutate(sampler_mGamma=future_map(sampler_result, "mGamma", .progress = TRUE),
                             sampler_vip=future_map(sampler_mGamma, ~apply(.x, 2, mean, .progress = TRUE)),
                             blma_vip=future_map(blma_result, "vinclusion_prob", .progress = TRUE),
                             error=future_map2_dbl(sampler_vip, blma_vip, ~sum((.x - .y)^2)))
parameters_tbl %>% select(data_set_name, prior, model_prior, error) %>% View
...
parameters_tbl %>% filter(data_set_name == "Kakadu") %>% select(prior, model_prior, sampler_vip) -> sampler_tbl
sampler_tbl %>% pmap(~ ggplot(tibble(prob=..3)) + geom_col(aes(x=1:length(prob), y=prob)) + ggtitle("Kakadu", subtitle = str_c("Variable inclusion for the ", ..1," prior with the ", ..2, " model prior")) + xlab("Covariates") + ylab("Inclusion probability"))
```
