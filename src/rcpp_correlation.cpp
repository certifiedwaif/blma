...
List blma(NumericVector vy, NumericMatrix mX, std::string prior,
  ...
  List result = blma_cpp(vy_m, mX_m, prior, modelprior, modelpriorvec_m, 
		  	 intercept_col - 1, bNatural_Order, bIntercept, bCentre, cores);
  return result;
}
...
//' @export
// [[Rcpp::export]]
List blma_fixed(NumericVector vy, NumericMatrix mX, NumericMatrix mZ, 
		std::string prior,
		std::string modelprior = "uniform",
		Nullable<NumericVector> modelpriorvec = R_NilValue,
                const unsigned int cores = 1) {
